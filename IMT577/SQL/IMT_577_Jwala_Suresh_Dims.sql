...
create or replace table IMT577_DW_JWALA_SURESH.PUBLIC.DIM_DATE (
...
select DATE_PKEY,
	...
	EFFECTIVE_DATE,
	EXPIRA_DATE
from --( select to_date('01-25-2019 23:25:11.120','MM-DD-YYYY HH24:MI:SS.FF') as DD, /*<<Modify date for preferred table start date*/    
...
SELECT 
    IFNULL(ChannelID,-1)
    ,IFNULL(s.ChannelCategoryID,-1)
    ,IFNULL(Channel,'Unknown') AS ChannelName
    ,IFNULL(ChannelCategory,'Unknown')
FROM STAGE_CHANNEL s 
JOIN STAGE_CHANNELCATEGORY c
...
CREATE OR REPLACE TABLE IMT577_DW_JWALA_SURESH.PUBLIC.Dim_Product(
    ...
    ,ProductProfitMarginUnitProfit FLOAT NOT NULL
);
...
SELECT
    IFNULL(ProductID,-1)
    ,IFNULL(s.ProductTypeID,-1)
    ,IFNULL(p.ProductCategoryID,-1)
    ,IFNULL(Product,'Unknown')
    ,IFNULL(ProductType,'Unknown')
    ,IFNULL(ProductCategory,'Unknown')
    ,IFNULL(Color,'Unknown')AS ProductColor
    ,IFNULL(Style,'Unknown') AS ProductStyle
    ,IFNULL(Weight,-1.0) AS ProductWeight
    ,IFNULL(Price,-1.0)
    ...
    ,IFNULL((((Price - Cost)/ Price)),-1.0) AS ProductProfitMarginUnitProfit
FROM STAGE_PRODUCT s 
JOIN STAGE_PRODUCTTYPE p 
JOIN STAGE_PRODUCTCATEGORY c
ON s.ProductTypeId = p.ProductTypeID AND 
...
FROM Dim_Location d 
JOIN STAGE_STORE s
WHERE d.PostalCode = s.PostalCode 
...
SELECT
    ...
    ,IFNULL(PhoneNumber,'Unknown')
    ,IFNULL(EmailAddress,'Unknown') AS Email
FROM Dim_Location d 
JOIN STAGE_RESELLER r
WHERE d.PostalCode = r.PostalCode AND 
d.Address = r.Address ;
...
SELECT
    IFNULL(DimLocationID,-1) AS DimLocationID
    ,IFNULL(CustomerID,'Unknown') AS CustomerID
    ,IFNULL(concat(FirstName,' ',LastName),'Unknown') AS FullName
    ,IFNULL(FirstName,'Unknown') AS FirstName
    ,IFNULL(LastName,'Unknown') AS LastName
    ,IFNULL(Gender,'Unknown') AS Gender
    ,IFNULL(EmailAddress,'Unknown') AS EmailAddress
    ,IFNULL(PhoneNumber,'Unknown') AS PhoneNumber
FROM Dim_Location d 
JOIN STAGE_CUSTOMER c
WHERE d.PostalCode = c.PostalCode AND 
d.Address = c.Address ;
